...
use app\merchant\model\ump\StoreCouponUser;
use app\merchant\model\wechat\WechatUser;
...
use app\merchant\model\routine\RoutineFormId;
...
use app\merchant\model\user\UserBill;
/**
 * 订单管理Model
 * Class StoreOrder
...
class StoreOrder extends ModelBasic
{
    ...
    public static function orderCount(){
        $data['wz']=self::statusByWhere(0,new self())->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['wf']=self::statusByWhere(1,new self())->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['ds']=self::statusByWhere(2,new self())->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['dp']=self::statusByWhere(3,new self())->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['jy']=self::statusByWhere(4,new self())->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['tk']=self::statusByWhere(-1,new self())->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['yt']=self::statusByWhere(-2,new self())->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['general']=self::where(['pink_id'=>0,'combination_id'=>0,'seckill_id'=>0,'bargain_id'=>0])->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['pink']=self::where('pink_id|combination_id','>',0)->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['seckill']=self::where('seckill_id','>',0)->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        $data['bargain']=self::where('bargain_id','>',0)->where(['mer_id'=>Session::get('adminInfo')['id']])->count();
        return $data;
    }
...
//        $model = $model->where('combination_id',0);
        ...
        if($where['mer_id'] != '') $model = $model->where($aler.'mer_id',$where['mer_id']);
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function setEchatWhere($where,$status=null,$time=null){
        $model=self::statusByWhere($where['status'])->where('mer_id',$where['mer_id']);
