...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
    ...
    public static function getbadge($where,$type){
        ...
        $stock1=self::getModelTime($where,new self())->where('stock','<',$replenishment_num)->where('mer_id',Session::get("adminInfo")['id'])->column('stock');
        $sum_stock=self::where('stock','<',$replenishment_num)->where('mer_id',Session::get("adminInfo")['id'])->column('stock');
        $stk=[];
        ...
        return [
            [
                ...
                'sum'=>self::where('mer_id',Session::get("adminInfo")['id'])->sum('stock'),
            ...
            [
                ...
                'sum'=>self::where('is_new',1)->where('mer_id',Session::get("adminInfo")['id'])->sum('stock'),
                'class'=>'fa fa-line-chart',
            ],
            [
                'name'=>'活动商品',
                'field'=>'件',
                'count'=>self::getModelTime($where,$StoreOrderModel)->where('mer_id',Session::get("adminInfo")['id'])->sum('total_num'),
                'content'=>'活动商品总数',
                'background_color'=>'layui-bg-green',
                'sum'=>$StoreOrderModel->where('mer_id',Session::get("adminInfo")['id'])->sum('total_num'),
    ...
    public static function setWhereType($model,$type){
        ...
        $model->where('mer_id',Session::get("adminInfo")['id']);
        return $model;
    }
    /*
    ...
    public static function getMaxList($where){
        ...
        $model=StoreOrder::alias('a')->join('StoreOrderCartInfo c','a.id=c.oid')->join('__STORE_PRODUCT__ b','b.id=c.product_id')->where('a.mer_id',Session::get("adminInfo")['id']);
        $list=self::getModelTime($where,$model,'a.add_time')->where('a.mer_id',Session::get("adminInfo")['id'])->group('c.product_id')->order('p_count desc')->limit(10)
    ...
    public static function ProfityTop10($where){
        $classs=['layui-bg-red','layui-bg-orange','layui-bg-green','layui-bg-blue','layui-bg-cyan'];
        $model=StoreOrder::alias('a')->join('StoreOrderCartInfo c','a.id=c.oid')->join('__STORE_PRODUCT__ b','b.id=c.product_id')->where('a.mer_id',Session::get("adminInfo")['id']);
        $list=self::getModelTime($where,$model,'a.add_time')->where('a.mer_id',Session::get("adminInfo")['id'])->group('c.product_id')->order('profity desc')->limit(10)
    ...
    public static function getLackList($where){
        $replenishment_num = SystemConfig::getValue('replenishment_num');
        $replenishment_num = $replenishment_num > 0 ? $replenishment_num : 20;
        $list=self::where('stock','<',$replenishment_num)->where('mer_id',$where['mer_id'])->field(['id','store_name','stock','price'])->page((int)$where['page'],(int)$where['limit'])->order('stock asc')->select();
        if(count($list)) $list=$list->toArray();
        $count=self::where('stock','<',$replenishment_num)->where('mer_id',$where['mer_id'])->count();
    ...
    public static function getnegativelist($where){
        $list=self::alias('s')->join('StoreProductReply r','s.id=r.product_id')
            ...
            ->where('s.mer_id',$where['mer_id'])
            ->order('count desc')
            ->group('r.product_id')
            ->select();
        if(count($list)) $list=$list->toArray();
        $count=self::alias('s')->join('StoreProductReply r','s.id=r.product_id')->where('s.mer_id',$where['mer_id'])->group('r.product_id')->where('r.product_score',1)->count();
        return ['count'=>$count,'data'=>$list];
    }
    public static function TuiProductList(){
        $perd=StoreOrder::alias('s')->join('StoreOrderCartInfo c','s.id=c.oid')
            ...
            ->group('c.product_id')
            ->where('s.status',-1)
            ->where('s.mer_id',Session::get("adminInfo")['id'])
            ->order('count desc')
            ->limit(10)
            ->select();
