...
use app\merchant\model\store\StoreProductRelation;
...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        //出售中产品
        $onsale =  ProductModel::where(['mer_id'=>$this->adminId,'is_show'=>1,'is_del'=>0])->count();
        //待上架产品
        $forsale =  ProductModel::where(['mer_id'=>$this->adminId,'is_show'=>0,'is_del'=>0])->count();
        //仓库中产品
        $warehouse =  ProductModel::where(['mer_id'=>$this->adminId,'is_del'=>0])->count();
        //已经售馨产品
        ...
        //回收站
        $recycle =  ProductModel::where(['mer_id'=>$this->adminId,'is_del'=>1])->count();
...
//        return $this->fetch('public/common_form');
        $field = [
            ...
            Form::select('type','产品类型')->setOptions(function(){
                $menus[] = ['value'=>1,'label'=>'普通产品','disabled'=>false];//,'disabled'=>$menu['pid']== 0];
                $menus[] = ['value'=>2,'label'=>'游玩产品','disabled'=>false];
                return $menus;
            ...
            Form::frameImageOne('image','产品主图片(305*305px)',Url::build('merchant/widget.images/index',array('fodder'=>'image')))->icon('image')->width('100%')->height('500px'),
            Form::frameImages('slider_image','产品轮播图(640*640px)',Url::build('merchant/widget.images/index',array('fodder'=>'slider_image')))->maxLength(5)->icon('images')->width('100%')->height('500px')->spin(0),
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::select('type','产品类型',(string)$product->getData('type'))->setOptions(function(){
                $menus[] = ['value'=>1,'label'=>'普通产品','disabled'=>false];//,'disabled'=>$menu['pid']== 0];
                $menus[] = ['value'=>2,'label'=>'游玩产品','disabled'=>false];
                return $menus;
            })->filterable(1),
            ...
            Form::frameImageOne('image','产品主图片(305*305px)',Url::build('merchant/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image')->width('100%')->height('500px'),
            Form::frameImages('slider_image','产品轮播图(640*640px)',Url::build('merchant/widget.images/index',array('fodder'=>'slider_image')),json_decode($product->getData('slider_image'),1) ? : [])->maxLength(5)->icon('images')->width('100%')->height('500px'),
