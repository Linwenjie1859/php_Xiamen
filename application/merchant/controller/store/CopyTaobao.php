...
use app\merchant\model\system\SystemAttachmentCategory;
...
class CopyTaobao extends AuthController
{
    ...
    */
            ...
            case 0:
                foreach ($images as $item){
                    ...
                    if(is_array($uploadValue)){
                        ...
                        //写入数据库
                        if(!$uploadValue['is_exists'] && $AttachmentCategoryId) SystemAttachment::attachmentAdd($uploadValue['name'],$uploadValue['size'],$uploadValue['mime'],$imagePath,$imagePath,$AttachmentCategoryId);
                        //组装数组
    ...
    public function downloadImage($url = '', $name = '', $type = 0, $timeout = 30, $w = 0, $h = 0)
    {
        ...
        $saveFilePath = ROOT_PATH . 'public' . DS . 'uploads' . DS . 'attach'  . DS . $date_dir. DS; //自动下载保存的文件目录
        if(!$name)
        {
            $f_arr = $this->getImageExtname($url);
            $name = $f_arr['file_name'];
            if($name)
            {
                $exits_name = $saveFilePath . md5($url) . '.' . $f_arr['ext_name'];
                if(file_exists($exits_name))
                {
                    $img_info = getimagesize($exits_name);
                    return ['path' => '/public/uploads/attach/' . $date_dir . '/' . $f_arr['file_name'], 'name' => $f_arr['file_name'], 'size' => '0', 'mime' => $img_info['mime'], 'is_exists' => true];
                }
            }
        }
        if(!$name || !$url) return '';
        $path = $saveFilePath . $name;
        //获取远程文件所采用的方法
        if ($type) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
        ...
        if($w > 0 || $h > 0)
        {
            $res = $this->resizeImage($saveFilePath,$name,$w,$h);
            return ['path' => '/public/uploads/attach/' . $date_dir . '/' . $res['file'], 'name' => $res['file'], 'size' => $size, 'mime' => $res['mime'], 'is_exists' => false];
        }else{
            $img_info = getimagesize($saveFilePath . $name);
            return ['path' => '/public/uploads/attach/' . $date_dir . '/' . $name, 'name' => $name, 'size' => $size, 'mime' => $img_info['mime'], 'is_exists' => false];
        }
    }
    //获取即将要下载的图片扩展名
