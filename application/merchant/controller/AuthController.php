...
use app\merchant\model\store\StoreMerchant;
...
class AuthController extends SystemBasic
{
    ...
    {
        parent::_initialize();
        if(!StoreMerchant::hasActiveAdmin()) return $this->redirect('Login/index');
        try{
            $adminInfo = StoreMerchant::activeAdminInfoOrFail();
        }catch (\Exception $e){
...
//        HookService::listen('admin_visit',$this->adminInfo,'system',false,SystemBehavior::class);
    ...
    protected function getActiveAdminInfo()
    {
        ...
        $adminInfo = StoreMerchant::getValidAdminInfoOrFail($adminId);
        if(!$adminInfo) $this->failed(SystemAdmin::getErrorInfo('请登陆!'));
        $this->adminInfo = $adminInfo;
        StoreMerchant::setLoginInfo($adminInfo);
        return $adminInfo;
    }
}