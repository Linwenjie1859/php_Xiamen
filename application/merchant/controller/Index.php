...
use app\merchant\model\order\StoreOrder as StoreOrderModel;//订单
use app\merchant\model\user\UserExtract as UserExtractModel;//分销
use app\merchant\model\user\User as UserModel;//用户
use app\merchant\model\store\StoreProductReply as StoreProductReplyModel;//评论
use app\merchant\model\store\StoreProduct as ProductModel;//产品
...
//            'role_name'=>SystemRole::where('id',$roles[0])->field('role_name')->find()
    ...
    public function main()
    {
        ...
        $topData['stockProduct'] = StoreProduct::where('stock','<=',$replenishment_num)->where('is_show',1)->where('is_del',0)->where('mer_id',$this->adminId)->count();

        //订单数->昨日
        $now_day_order_p = StoreOrderModel::where('paid',1)->whereTime('pay_time','yesterday')->where('mer_id',$this->adminId)->count();
        $pre_day_order_p = StoreOrderModel::where('paid',1)->where('pay_time','gt',$pre_day)->where('pay_time','lt',$now_day)->where('mer_id',$this->adminId)->count();
        $first_line['d_num'] = [
        ...
        //交易额->昨天
        $now_month_order_p = StoreOrderModel::where('paid',1)->whereTime('pay_time','yesterday')->where('mer_id',$this->adminId)->sum('pay_price');
        $pre_month_order_p = StoreOrderModel::where('paid',1)->where('pay_time','gt',$beforyester_day)->where('pay_time','lt',$pre_day)->where('mer_id',$this->adminId)->sum('pay_price');
        $first_line['d_price'] = [
        ...
        //交易额->月
        $now_month_order_p = StoreOrderModel::where('paid',1)->whereTime('pay_time','month')->where('mer_id',$this->adminId)->sum('pay_price');
        $pre_month_order_p = StoreOrderModel::where('paid',1)->where('pay_time','gt',$pre_month)->where('pay_time','lt',$now_month)->where('mer_id',$this->adminId)->value('sum(pay_price)');
        $first_line['m_price'] = [
            'data' => $now_month_order_p > 0 ? $now_month_order_p : 0,
            'percent' => abs($now_month_order_p - $pre_month_order_p),
            'is_plus' => $now_month_order_p - $pre_month_order_p > 0 ? 1 : ($now_month_order_p - $pre_month_order_p == 0 ? -1 : 0)
        ...
        //本月订单总数
        $now_order_info_c = StoreOrderModel::where('add_time','gt',$now_month)->where('mer_id',$this->adminId)->count();
        $pre_order_info_c = StoreOrderModel::where('add_time','gt',$pre_month)->where('add_time','lt',$now_month)->where('mer_id',$this->adminId)->count();
        $order_info['first'] = [
        ...
        $now_order_info_c = StoreOrderModel::where('add_time','gt',$pre_month)->where('add_time','lt',$now_month)->where('mer_id',$this->adminId)->count();
        $pre_order_info_c = StoreOrderModel::where('add_time','gt',$second_pre_month)->where('add_time','lt',$second_now_month)->where('mer_id',$this->adminId)->count();
        $order_info["second"] = [
            'data' => $now_order_info_c ? $now_order_info_c : 0,
            'percent' => abs($now_order_info_c - $pre_order_info_c),
            'is_plus' => $now_order_info_c - $pre_order_info_c > 0 ? 1 : ($now_order_info_c - $pre_order_info_c == 0 ? -1 : 0)
    ...
    public function Jnotice()
    {
        header('Content-type:text/json');
        $data = [];
        $data['ordernum'] = StoreOrderModel::statusByWhere(1)->where('mer_id',$this->adminId)->count();//待发货
        $replenishment_num = SystemConfig::getValue('store_stock') > 0 ? SystemConfig::getValue('store_stock') : 2;//库存预警界限
        $data['inventory'] = ProductModel::where('stock','<=',$replenishment_num)->where('is_show',1)->where('is_del',0)->where('mer_id',$this->adminId)->count();//库存
