...
class CouponsApi extends AuthController
{
    ...
    public function get_coupon_rope($couponId = 0){
        ...
        $couponUser['add_time'] = date('Y/m/d',$couponUser['add_time']);
        $couponUser['end_time'] = date('Y/m/d',$couponUser['end_time']);
    ...
    /**
     * @Modify: Mr. Lin
     * @function: 通过$cid获得优惠券信息
     * @instructions: 
     * @param {type} 
     * @return: JSON
     */
    public function get_coupon_info($cid){
        $coupon=StoreCoupon::getCouponInfo($cid);
        $coupon['add_time'] = date('Y/m/d',$coupon['add_time']);
        return JsonService::successful($coupon);
    }
}